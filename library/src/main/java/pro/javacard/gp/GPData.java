...
public final class GPData {
            ...
            if (isdd.isPresent()) {
                ...
                for (TLV vt : isdd.get().children()) {
                    if (vt.tag().equals(Tag.ber(0x06))) {
                        final String oid = logAndGetOidFromByteArray(vt.tag().bytes(), vt.value());
                        if ("1.2.840.114283.1".equals(oid)) {
                            System.out.println("-> Global Platform card");
                        }
                    } else if (vt.tag().equals(Tag.ber(0x60))) {
                        final String oid = logAndGetOidFromByteArray(vt.tag().bytes(),
                                vt.children().get(0).value()); // 6X are constructed tags
                        if (oid.startsWith("1.2.840.114283.2")) {
                            final var p = oid.substring("1.2.840.114283.2.".length()).split("\\.");
                        ...
                        for (TLV ot : vt.children()) {
                            final var oidBytes = ot.value();
                            final String oid = logAndGetOidFromByteArray(ot.tag().bytes(), oidBytes);
                            // This also works with the invalid encoding for SCP80 i=00
                            if (oid.startsWith("1.2.840.114283.4.")) {
                                final byte[] scp = Arrays.copyOfRange(oidBytes, oidBytes.length - 2, oidBytes.length);
                        ...
                        if (oid.startsWith("1.3.6.1.4.1.42.2.110.1")) {
                            final var p = oid.substring("1.3.6.1.4.1.42.2.110.1.".length());
                            if (p.length() == 1) {
    ...
    public enum LFDBH {
        ...
        LFDBH(final int byteValue, final String algo) {
            this.value = byteValue;
            this.algo = algo;
        }

        public static Optional<LFDBH> byValue(final int byteValue) {
            return Arrays.stream(values()).filter(e -> e.value == byteValue).findFirst();
        }

        public static Optional<LFDBH> fromString(final String s) {
            return Arrays.stream(values()).filter(e -> e.name().equalsIgnoreCase(s)).findFirst();
        }

        public static List<LFDBH> fromBytes(final byte[] v) {
            final var r = new ArrayList<LFDBH>();
            for (var i = 0; i < v.length; i++) {
                final var j = i; // TODO: IntStream.range() ?
    ...
    public enum SIGNATURE implements BitField<SIGNATURE> {
        ...
        SIGNATURE(final BitField.Def def) {
            this.def = def;
        }
    ...
    public static void dump(final APDUBIBO channel) throws IOException, GPException {
        ...
        // IIN
        final byte[] iin = getData(channel, 0x00, 0x42, "IIN", false);
        if (iin != null) {
            System.out.println("IIN: " + HexUtils.bin2hex(iin));
        }
        // CIN
        final byte[] cin = getData(channel, 0x00, 0x45, "CIN", false);
        if (cin != null) {
            System.out.println("CIN: " + HexUtils.bin2hex(cin));
        }

        // KDD
        final byte[] kdd = getData(channel, 0x00, 0xCF, "KDD", false);
        if (kdd != null) {
            System.out.println("KDD: " + HexUtils.bin2hex(kdd));
        }
        // SSC
        final byte[] ssc = getData(channel, 0x00, 0xC1, "SSC", false);
        if (ssc != null) {
            System.out.println("SSC: " + HexUtils.bin2hex(ssc));
        }
        // Print Card Data
        System.out.println("Card Data: ");
        final byte[] cardData = getData(channel, 0x00, 0x66, "Card Data", false);
        if (cardData != null) {
            pretty_print_card_data(cardData);
        }
        // Print Card Capabilities
        System.out.println("Card Capabilities: ");
        final byte[] cardCapabilities = getData(channel, 0x00, 0x67, "Card Capabilities", false);
        if (cardCapabilities != null) {
            pretty_print_card_capabilities(cardCapabilities);
        }

        // Print Key Info Template
        final byte[] keyInfo = fetchKeyInfoTemplate(channel);
        if (keyInfo != null) {
    ...
    public static String sw2str(final int sw) {
        final var msg = GPData.sw.get(sw);
        if (msg == null) {
            return "0x%04X".formatted(sw);
        }
        return "0x%04X (%s)".formatted(sw, msg);
    }

    public static String oid2string(final byte[] oid) {
        ...
        final var realoid = ASN1ObjectIdentifier.fromContents(oid);
    ...
    private static String logAndGetOidFromByteArray(final byte[] tag, final byte[] tlv) {
        final String oid = oid2string(tlv);
        System.out.println("Tag " + new BigInteger(1, tag).toString(16) + ": " + oid);
        return oid;
    }

    // Big-endian bytes to int (1-4 bytes)
    private static int be2int(final byte[] b) {
        var r = 0;
        for (byte v : b) {
            r = (r << 8) | (v & 0xFF);
        }
    ...
    public static String oid2version(final byte[] bytes) throws GPDataException {
        final String oid = oid2string(bytes);
