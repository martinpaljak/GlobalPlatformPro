...
public class TestBitField {
    ...
    public void testEncodePrivileges() {
        var privs = EnumSet.of(Privilege.SecurityDomain, Privilege.CardLock);
        byte[] encoded = BitField.encode(privs, 3);
        Assert.assertEquals(encoded, HexUtils.hex2bin("900000"));
    }

    @Test
    public void testRoundTrip() {
        byte[] v = HexUtils.hex2bin("9EFE80");
        Set<Privilege> privs = BitField.parse(Privilege.class, v, 1, 3);
        byte[] encoded = BitField.encode(privs, 3);
    ...
    public void testInvalidLength() {
        byte[] v = HexUtils.hex2bin("8000"); // 2 bytes, invalid
    ...
    public void testRfuBitsThrow() {
        ...
        byte[] v = HexUtils.hex2bin("800007");
        BitField.parse(Privilege.class, v, 1, 3);
    }
    ...
    public void testLaxParseNoLengthCheck() {
        ...
        byte[] v = HexUtils.hex2bin("80");
        Set<Privilege> privs = BitField.parse(Privilege.class, v);
        Assert.assertTrue(privs.contains(Privilege.SecurityDomain));
    }
    ...
    public void testSignatureParse() {
        ...
        Set<GPData.SIGNATURE> sigs = BitField.parse(GPData.SIGNATURE.class, v);
    ...
    public void testByteMask() {
        ...
        var def = BitField.byte_mask(0, 0xC0);
